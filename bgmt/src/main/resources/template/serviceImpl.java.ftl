...
import ${package.ServiceImpl?substring(0,(package.ServiceImpl)?length-12)}exception.ApiException;
...
import ${package.ServiceImpl?substring(0,(package.ServiceImpl)?length-12)}utils.HcUtil;
/**
 * <p>
 * ${table.comment!} 服务实现类
 * </p>
...
public class ${table.serviceImplName} extends ${superServiceImplClass}<${table.mapperName}, ${entity}> implements ${table.serviceName} {
    ...
    @Override
    public IPage<${entity}> findPage(Pageable pageable) throws ApiException {
        IPage<${entity}> retPage;
        try {
            Page<${entity}> page = new Page<>(pageable.getPageNumber(), pageable.getPageSize());
    ...
    public void add(${entity} ${entity?uncap_first}) throws ApiException {
        try {
            if (StringUtils.isBlank(${entity?uncap_first}.getId())) {
                ${entity?uncap_first}.setId(HcUtil.getUUID(true));
            }
